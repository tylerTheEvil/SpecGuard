...
.. deprecated:: SUPERSEDED (2026-08-09)
    This combined-taxonomy rescore was a scoring-side workaround for the
    pre-typed extractor, whose single MENTIONS type covered components AND
    standards. The extractor now proposes ``REFERS_TO`` as a first-class
    typed edge and ``experiments/edge_extraction_eval.py`` scores it
    directly against the builder's 21 standards edges, so this workaround
    is no longer the current method. Kept (with its committed input
    artifacts) solely so the historical combined-taxonomy figures remain
    reproducible; it only applies to PRE-FIX artifacts whose MENTIONS logs
    still contain standard-targeted proposals. Do not cite its output for
    the current pipeline — use the per-type numbers from the typed eval.
...
# Pinned to the FROZEN pre-fix artifacts only (the prompt-variant grid, whose
# baseline_run1 files are verbatim copies of the pre-fix canonical runs). The
# canonical results/edge_extraction_eval*.json files were regenerated by the
# typed extractor and no longer contain the pre-fix data this historical
# rescore applies to — including them would silently mix extractor versions.
RESCORABLE = [
...
def main() -> int:
    ...
    hdr = (
        f"{'run':52s} {'P(comp)':>8s} {'R(comp)':>8s} "
        f"{'P(comb)':>8s} {'R(comb)':>8s}  FP comp/std/oov"
    )
    print(hdr)
    for r in runs:
